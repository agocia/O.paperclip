...
> 若 USB 被拔除、Wi‑Fi tunnel 中斷，App 會主動偵測掉線、立即停止目前模擬，並在側邊欄顯示提示後自動嘗試重連。
...
| **搖桿** | 用方向鍵或 WASD 即時推動目前位置 |

> `固定路線` 已不再出現在模式選擇器中，請改從右側「匯入與收藏」欄位匯入或套用。
...
> 也可在搜尋欄輸入地址或地名直接跳轉。
> 路線草稿與活動中的藍線都會在地圖上標示起點、終點；閉圈會以單一「起點／終點」標記顯示。
> 活動中的路線會即時顯示剩餘時間；尚未開始時則顯示單趟預估時間。

### 收藏內容

右側「匯入與收藏」欄位可直接儲存目前點位或路線，之後點「套用」時會自動切到相容模式：

- 收藏點位 → 自動切到「定點」
- 收藏 A-B 路線 → 自動切到「A-B」
- 收藏多點或固定路線 → 自動切到「多點」
- 收藏迴圈 → 自動切到「多點」，並自動啟用閉圈
...
A: 這是正常行為。App 會主動檢查 USB 裝置是否仍在線上；一旦確認掉線，就會立即停止模擬，並在側邊欄提示「已偵測裝置掉線，模擬已停止」，接著自動嘗試重連。
